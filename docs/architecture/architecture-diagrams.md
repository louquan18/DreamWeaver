...
> ⚠️ 本文图表描绘**目标架构**，部分领先于当前实现，尤其：
> - 图 4「Checkpoint 恢复」画的是 PostgreSQL 持久化，**当前实为内存态 MemorySaver，不支持重启恢复**；
> - 图 5 / 图 9「多模型路由」画的是 Claude/GPT/Opus 等，**当前实际仅接入 MiMo 单模型**；
> - 图 8「完整章节生成时序」画的是同步执行一致性/评审，**当前流式接口为 draft 流完即返回、后续节点后台旁路执行**。
>
> 各能力的真实实现状态以 [STATUS.md](../STATUS.md) 为唯一权威。图 2「Agent 工作流状态机」与代码一致（rewrite → review 循环）。
